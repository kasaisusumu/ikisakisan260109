...
function HomeContent() {
  ...
  // const [isTrialInitialized, setIsTrialInitialized] = useState(false); // ★お試しモードの初期化フラグ
...
const onTimelineDrop = async (e: React.DragEvent, targetIndex: number) => {
    ...
    if (!isTrial) { // ★お試しモード時は並び順をDBに保存しない
        ...
        planSpots.forEach(async (spot) => {
            if (!spot.image_url && !attemptedImageFetch.current.has(spot.id)) {
                ...
                if (existingSpotWithImage) {
                     ...
                     if (roomId && !isTrial) supabase.from('spots').update({ image_url: existingSpotWithImage.image_url }).eq('id', spot.id).then();
                     return;
                }
                const url = await fetchSpotImage(spot.name);
                if (url) {
                    setPlanSpots(prev => prev.map(s => s.id === spot.id ? { ...s, image_url: url } : s));
                    if (roomId && !isTrial) { supabase.from('spots').update({ image_url: url }).eq('id', spot.id).then(); }
                }
            }
        });
    }
...
// ... 既存のコード ...
  ...
  const updateSpotStatus = async (spot: any, newStatus: string, day: number = 0) => {
      ...
      // DB更新
      if (!isTrial && spot.id && !String(spot.id).startsWith('spot-') && !String(spot.id).startsWith('ai-')) {
    try { 
...
// ★修正: Day変更時の処理（ID判定・通知・キャッシュ整合性を強化）
  const updateSpotDay = async (spot: any, newDay: number) => {
      ...
      // 4. データベース更新 (★ お試しモード以外かつ roomId がある場合のみ実行)
      if (roomId && !isTrial) {
...
const loadRoomData = async (rId: string) => {
    ...
    // 通常の読み込み（お試しモードの初回もここを通り、DBからテンプレートをロードする）
    try {
    // 1. スポットデータの取得（お試しモードでも最初はテンプレートとして読み込む）
    ...
    const { data: allVotes, error: votesError } = await supabase
      .from('votes') // ★ 'spot_votes' から 'votes' に修正
      .select('*')
      .eq('room_id', rId);
...
// ---------------------------------------------------------
 ...
 // リアルタイム同期 & 低コスト更新ロジック
  // ... (前略)
// ----------------------------------------------------
// ★ 追加: お試しモード中の変更をローカルストレージに自動同期するフック
// ----------------------------------------------------
...
useEffect(() => {
    ...
    // ★お試しモードの場合はここで処理を終了し、リアルタイム同期を起動させない
    if (isTrial) return;
...
// ★追加: 既存の同名スポットがあれば、メモ・リンク・金額・画像を引き継ぐ
 ...
 const removeSpot = async (spot: any) => {
    ...
    // 変更後
// 3. データベースから削除 (★ お試しモード以外かつ roomId がある場合のみ実行)
    if (roomId && !isTrial && !targetId.startsWith('temp-') && !targetId.startsWith('ai-')) {
...
// 2. データベースの更新
  ...
  }; // ★ ifの閉じ「}」と、handleSaveMemo関数の閉じ「}」の両方が必要
...
// 2. DBへの保存通信は「お試しモード以外」かつ「roomIdがある」場合のみ実行する
  // 2. DBへの保存通信は「お試しモード以外」かつ「roomIdがある」場合のみ実行する
    if (roomId && !isTrial) {
        ...
        if (error) {
            ...
            alert("設定の保存に失敗しました");
            return; // ★エラーが発生した場合はここで処理を止め、モーダルを閉じない
        }
    }

    // ★お試しモードの場合、または通常モードで保存が成功した場合はモーダルを閉じる
    setShowDateModal(false);
  };
                  ...
                  setPlanSpots(prevSpots => {
                      return prevSpots.map(s => {
                          ...
                          if (s.name === name && (s.description === "住所調査中..." || s.description === "住所調査中")) {
                             ...
                             // IDが確定している(DBにある)なら、DBも更新する
                              if (s.id && !String(s.id).startsWith('temp-') && roomId && !isTrial) { // ★お試しモードをブロック
                                  supabase.from('spots').update({ 
                                      description: newDesc,
                                      comment: newComment,
                                      image_url: newImage,
...
{/* ▲▲▲ 修正ここまで ▲▲▲ */}
    ...
    onClick={() => { 
        ...
        addSpot({ 
            ...
            url: selectedResult.url || generatedUrl,
            status: 'candidate' 
        }); 
        
        // ★修正: お試しモード時など、すでにパネルが閉じて prev が null になっている場合のエラーを回避
        setSelectedResult((prev: any) => {
            if (!prev) return null; // ← これを追加
            return {
                ...prev, 
                is_saved: true,
                url: prev.url || generatedUrl
            };
        }); 
    }} 
...
const ReservationButton = ({ spot, roomId, onUpdate, currentUser, compact = false, isTrial = false }: { spot: any, roomId: string, onUpdate: (s: any) => void, currentUser?: string, compact?: boolean, isTrial?: boolean }) => {
